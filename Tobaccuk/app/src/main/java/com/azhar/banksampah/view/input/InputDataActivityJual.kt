...
import com.azhar.banksampah.viewmodel.InputDataViewModelJual
import kotlinx.android.synthetic.main.activity_input_data_jual.*
import java.text.SimpleDateFormat
import java.util.*

class InputDataActivityJual : AppCompatActivity() {

    lateinit var inputDataViewModelJual: InputDataViewModelJual
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_input_data_jual)
        setStatusBar()
        setToolbar()
        setInitLayout()
        setInputData()
    ...
    private fun setInitLayout() {
        ...
        inputDataViewModelJual = ViewModelProvider(this, ViewModelProvider.AndroidViewModelFactory.getInstance(this.application)).get(InputDataViewModelJual::class.java)

        val arrayBahasa = ArrayAdapter(this@InputDataActivityJual, android.R.layout.simple_list_item_1, strKategori)
        ...
        inputTanggal.setOnClickListener { view: View? ->
            ...
            DatePickerDialog(
                this@InputDataActivityJual, date,
    ...
    private fun setInputData() {
        btnCheckout.setOnClickListener { v: View? ->
            ...
            if ( strTanggal.isEmpty() or (strKategori.size == 0) or (countBerat == 0) or (countHarga == 0)) {
                Toast.makeText(
                    this@InputDataActivityJual,
                    "Data tidak boleh ada yang kosong!",
                    Toast.LENGTH_SHORT
                ).show()
            } else {
                inputDataViewModelJual.addDataSampah(
                    strKategoriSelected,
                    countBerat,
                    countHarga,
                    strTanggal,
